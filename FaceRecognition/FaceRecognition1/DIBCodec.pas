...
//****************************************************************************//
// Version 1.4   Copyright/CopyLeft GPL (C) 1999 LEON Sébastien               //
//                                                                            //
// ## History : ##                                                            //
// Version 1.0 : (1/1999)                                                     //
...
// THIS IS THE INTERFACE OF MY "INC" PRECOMPRESSION FILTER (INCREMENTAL)
procedure SebINCFilter    ( Pt : Pointer ; Width, Height     : Integer ; Str  : TStream ; ToDo : Boolean);
...
Var
 V  : Array [0..NbV] of LongInt;   // Nb d'appels
 VN : Array [0..NbV] of String[14]; // Nom de fonction
{$ENDIF}
...
var // Optimized For 8 Bits per Pixels ! Not good for others resolutions.
...
Const
  BufSize = 1024; // Buffer de 3 * cette taille ! (en pile)
Type
  RVBBuf = packed record
    R, V, B : Byte;
  End;
...
var
  ...
  MODBf, W3   : Integer;
  YCur        : Pointer;
  pR, pV, pB  : ^Byte;   // Pointer to RVB values
  lR, lV, lB  :  Byte;   // Last RVB Value
  Buffer : array [0..BufSize-1] of RVBBuf;
Begin
  // Cette optimisation est intéressante uniquement pour le mode 24 bpp !
  W3    := (Width * 3);
  XInc  := (W3 + 3) AND NOT $3;  // Width_b
  MxBf  := (Width + BufSize - 1) DIV BufSize;
  MODBf := (Width MOD BufSize);
  // (en mémoire) faire [ la dérivée / l'incrémentation ] pour R, V, B :
  YCur  := Pt;
  For Y := 0 To (Height-1) Do Begin
    pR  := YCur; pV := P(I(pR) + 1); pB := P(I(pV) + 1);
    lR  := 0; lV := 0; lB := 0; // En début de ligne, on recommence en absolue !
    For Bf := 1 To MxBF Do Begin
      If (Bf = MxBf) Then W := MODBf Else W := BufSize;
      If ToDo Then Begin
        For X := 0 To (W-1) Do Begin // A mettre
          Buffer[X].R := pR^ - lR; lR := pR^; Inc(pR, 3);
          Buffer[X].V := pV^ - lV; lV := pV^; Inc(pV, 3);
          Buffer[X].B := pB^ - lB; lB := pB^; Inc(pB, 3);
        End;
        Str.WriteBuffer( Buffer, W * 3 );
      End Else Begin
        Str.ReadBuffer ( Buffer, W * 3 );
        For X := 0 To (W-1) Do Begin // A enlever
          lR := Buffer[X].R + lR; pR^ := lR; Inc(pR, 3);
          lV := Buffer[X].V + lV; pV^ := lV; Inc(pV, 3);
          lB := Buffer[X].B + lB; pB^ := lB; Inc(pB, 3);
        End;
      End;
    End;
...
Begin
  Msg := '';
  For Ms := 0 to NbV Do Msg := Msg + #10#13 + 'Fct : '+VN[Ms]+' -> '+IntToStr(V[Ms])+' appels.';
  MessageDlg('Statistiques d''appel : '+Msg,mtInformation,[mbOk], 0);
End;
