...
// 1, 4, 8 Bit(s) per Pixel.
  ...
  T256ColorsPalette = Packed Record
    Case Integer of
      ...
      1 : ( // Sometimes, more easy...
          Ver          : Word;
          Entries      : Word;
          Color        : array [0..255] of Integer
          );
  End;
  ...
  TDUPixelFormat = (
    DUpf1,           // 1  bit per pixel   => 2      colors (a palette)  => 1 plane   DIB
    DUpf4,           // 4  bits per pixel  => 16     colors (a palette)  => 4 planes  DIB
    DUpf8,           // 8  bits per pixel  => 256    colors (a palette)  => 8 planes  DIB
    DUpf15,          // 15 bits per pixel  => 32768  Colors (no palette) => 15 planes DIB
    DUpf16,          // 16 bits per pixel  => 65536  Colors (no palette) => 16 planes DIB
    DUpf24,          // 24 bits per pixel  => 16.7 M Colors (no palette) => 24 planes DIB
    DUpf32           // 24 bits per pixel  => 16.7 M Colors (no palette) => 24 planes DIB (DWORD ALIGNED !)
  );

  // A WORD ABOUT PENMODE :
  //
  // DIBUltra must stay (actually) a speed & lightweight library.
  // Of course we could implement every WinAPI modes but DIBUltra will be
  // as fat as Windows !
  // So I think it's not useful that DIBUltra supports the following penMode :
  //   DUpmMASKNOTPEN , DUpmMERGENOTPEN , DUpmBLACK , DUpmWHITE : because a user
  //   has just to change PenColor before drawing to emulate them.
  // The others will may be supported a day, but I think they are not really useful either :
  // (Have you ever use DUpmMASKPENNOT, DUpmMERGEPENNOT, DUpmNOTXORPEN, DUpmNOTMERGEPEN
  //  DUpmNOTMASKPEN and DUpmNOTCOPYPEN in your developper career ? Me : never )
  // I have change titles to more intuitive ones (Why Borland said MERGEPEN instead of ORPEN)
  // This library will soon run with Linux so I don't want to ever refer to Borland or
  // Microsoft conventions !

  TDUPenMode = ( (* Developpers : DO NOT Modifie this order until you know exactly what you are doing ! *)
    DUpmCOPY,        //Pixel is pen.
    DUpmNOT,         //Pixel is the inverse of the screen color.
    DUpmANDPEN,      //Pixel is (pen AND screen pixels)
    DUpmORPEN,       //Pixel is (pen OR  screen pixels)
    DUpmXORPEN,      //Pixel is (pen XOR screen pixels)
    DUpmALPHAPEN,    //Pixel is (pen ALPHA screen pixels) => Set PenAlpha before
    DUpmUSER,        //Pixel is (pen XOR screen pixels)   => Set PenCallBack before
    DUpmNOP          //Pixel remains unchanged.
  );

  TColorIndex    = TColor;

  TUnionDIBType  = (DUtTransparent, DUtAlpha, DUtAnimate, DUtLZH, DUtSysPalette, DUnoRLE, DUINC, DUUnUsed);
