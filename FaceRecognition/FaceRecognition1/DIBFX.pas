...
begin
  //à faire : test pour savoir si l'on a bien le même nbre de couleurs

//  If (Self.PixelFormat<>DUpf16) Then Raise Exception.Create('La DIBUltra à afficher par filtre Alpha doit être une image à 16 Bpp !');
//  If (Dest.PixelFormat<>DUpf16) Then Raise Exception.Create('L''image en Destination doit être une image à 16 Bpp !');
  If (Wipe.PixelFormat<>DUpf8) Then Raise Exception.Create('L''image guidant la transition doit être une image à 8 Bpp !');
...
@TRAITEMENT:
    ...
    CMP  DL, couleur      // Est-ce notre valeur à copier?
    JNZ   @INCREG         // Non, on passe à la valeur suivante
...
@INCREG:
    ...
    ADD  ESI, 3      // Déplacement des pointeurs
    ADD  EDI, 3      //   LEA  EDI, EDI + 3 marche aussi

    DEC     ECX              //LOOP @SUITEPIXEL   //ràZ pourquoi?
